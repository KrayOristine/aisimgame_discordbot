...
export class Database {
  ...
  _iDbDatabase: IDBDatabase | null;
  _upgrade:
    | ((transaction: Transaction, oldVersion: number, newVersion: number) => Promise<void> | void)
    | null = null;
  _close: (() => void) | null = null;
  _versionChange: (() => void) | null = null;
   ...
   */
  get iDbDatabase(): IDBDatabase | null {
    ...
    const promise = new Promise<Database>((resolve, reject) => {
      ...
      openDbRequest.onsuccess = (event) => {
        ...
        this._iDbDatabase.onclose = () => {
          ...
          if (this._close !== null) this._close();
          else this.onClose();
        };
        ...
        this._iDbDatabase.onversionchange = () => {
          ...
          if (this._versionChange !== null) this._versionChange();
          else this.onVersionChange();
        };
      ...
      openDbRequest.onupgradeneeded = (event) => {
        ...
        try {
          ...
          const upgradePromise =
            this._upgrade !== null
              ? this._upgrade(transaction, event.oldVersion, event.newVersion)
              : this.onUpgrade(transaction, event.oldVersion, event.newVersion);
