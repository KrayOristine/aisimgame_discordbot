...
const FandomGenesisScreen: React.FC<FandomGenesisScreenProps> = ({ onBack }) => {
  ...
  const isChunkSettingValid =
    estimatedTokens < MAX_CHUNK_TOKENS && chunkSize > 0 && overlap >= 0 && chunkSize > overlap;
    ...
    try {
      ...
      if (!arcs || arcs.length === 0) {
        setNotification({
          ...
          messages: ["AI không thể xác định được các phần truyện (Arc) nào từ tệp tóm tắt này."],
        });
        setArcList([]);
      } else {
        setArcList(arcs);
  ...
  const handleGenerateSelectedArcs = async () => {
    ...
    try {
      ...
      setNotification({
        isOpen: true,
        title: "Hoàn tất!",
        messages: [`AI đã tóm tắt và lưu thành công ${arcsToProcess.length} tệp .txt vào kho.`],
      });
  ...
  const handleSaveToBrowser = async () => {
    ...
    setNotification({
      isOpen: true,
      title: "Đã lưu!",
      messages: [`Đã lưu "${generatedResult.name}" vào kho lưu trữ của trình duyệt.`],
    });
    setGeneratedResult(null);
    await refreshSavedFiles();
  };
  ...
  const handleTrainFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const combinedName =
        validFiles.length > 1 ? `Tong_hop_${validFiles.length}_tep.txt` : validFiles[0].name;
        ...
        <div className="space-y-8">
          ...
          >
            <p className="text-slate-400 mb-4 text-sm">
              Cung cấp cho AI cái nhìn tổng quan về tác phẩm. Tệp tóm tắt này là **đầu vào bắt
              buộc** cho Bước 2.
            </p>
          ...
          >
            <p className="text-slate-400 mb-4 text-sm">
              Chọn tệp tóm tắt (`tom_tat_...`) từ kho, sau đó quét để lấy danh sách các Arc. Cuối
              cùng, chọn các Arc bạn muốn AI tóm tắt chi tiết.
            </p>
            <div className="bg-slate-900/30 p-4 rounded-md border border-slate-700">
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-center mb-4">
                ...
                {selectedSummary && (
                  <p className="text-sm text-slate-300">
                    Đã chọn: <span className="font-semibold">{selectedSummary.name}</span>
                  </p>
                )}
              </div>
              <div className="flex justify-end">
                <Button
                  ...
                  className="cursor-pointer w-auto! text-base! py-2! px-6!"
                >
                  {loadingStates.arc && arcProcessingProgress.status === "extracting_arcs" ? (
                    "Đang phân tích..."
                  ) : (
                    <>
            ...
            {arcList.length > 0 && (
              <div className="mt-6 bg-slate-900/30 p-4 rounded-md border border-slate-700 animate-fade-in">
                    ...
                    <div className="mt-4 p-3 bg-slate-900/50 rounded-md text-sm text-slate-300 animate-fade-in">
                      ...
                      {arcProcessingProgress.status === "done" && (
                        <p className="font-semibold text-green-400">
                          Hoàn tất! Đã tạo và lưu {arcProcessingProgress.total} tệp .txt vào kho.
                        </p>
                      )}
                    </div>
                  )}
                ...
                <div className="mt-6 flex justify-end">
                  ...
                  >
                    {loadingStates.arc && arcProcessingProgress.status === "summarizing" ? (
                      "Đang xử lý..."
                    ) : (
                      <>
                        <Icon name="magic" className="w-5 h-5 mr-2" />
          ...
          >
            <p className="text-slate-400 mb-4 text-sm">
              Công cụ này giúp bạn xử lý một tệp .txt lớn (nguyên tác), cắt nhỏ (chunking), tạo
              vector và đóng gói thành một tệp Dataset (.json). Tệp .json này được tối ưu hóa để làm
              'Kiến thức nền', giúp AI truy xuất thông tin hiệu quả và chính xác hơn.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-slate-800 p-3 rounded-md">
                  ...
                  >
                    ...
                    <span>
                      ...
                      {isChunkSettingValid ? "Cài đặt hợp lệ." : "Vượt giới hạn!"}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-900/30 p-4 rounded-md border border-slate-700 flex flex-col justify-between">
                      ...
                      <StyledInput
                        ...
                        onChange={(e) =>
                          setSourceTrainFile((prev) =>
                            prev ? { ...prev, name: `${e.target.value}.txt` } : null,
                          )
                        }
                      />
                    </div>
                ...
                <Button
                  ...
                  variant="warning"
                  disabled={!sourceTrainFile || !isChunkSettingValid || loadingStates.training}
              ...
              <>
                ...
                <p className="text-sm text-amber-300 mt-2 flex items-start">
                  ...
                  <span>
                    ...
                    <span className="font-mono bg-slate-700 px-1 rounded">tom_tat_...</span>. Vui
                    lòng không đổi tên tệp này để AI có thể nhận diện và ưu tiên xử lý chính xác.
                  </span>
                </p>
            ...
            {trainedDataset && (
              <>
                ...
                <p className="text-sm text-slate-300 mt-1">
                  ...
                  <span className="font-bold">{trainedDataset.metadata.totalChunks}</span> chunks
                  trong một Dataset.
                </p>
                <p className="text-sm text-amber-300 mt-2 flex items-start">
                  <Icon name="info" className="w-4 h-4 inline mr-2 mt-0.5 shrink-0" />
                  <span>
                    Lưu Dataset này vào kho để sử dụng làm "Kiến thức nền" trong màn hình "Kiến tạo
                    Thế giới".
                  </span>
                </p>
                <div className="flex gap-4 mt-4">
